...
void wxsBaseProperties::BuildSetupWindowCode(wxString& Code,const wxString& WindowName,wxsCodingLang Language)
{
    switch ( Language )
    {
        case wxsCPP:
        {
            ...
            if ( !m_ToolTip.empty()  ) Code << VarAccess << _T("SetToolTip(") << wxsGetWxString(m_ToolTip) << _T(");\n");
            if ( !m_HelpText.empty() ) Code << VarAccess << _T("SetHelpText(") << wxsGetWxString(m_HelpText) << _T(");\n");
            return;
        }
    }
