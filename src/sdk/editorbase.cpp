  ...
  if(!noeditor && wxGetKeyState(WXK_CONTROL))
    {
      ...
      wxMenu* switchto = CreateContextSubMenu(idSwitchTo);
      if(switchto)
      popup->Append(idSwitchTo, _("Switch to...")
                      , switchto);
      }
...
void EditorBase::OnContextMenuEntry(wxCommandEvent& event)
{
  ...
  if (id == idCloseMe)
	...
	wxLaunchDefaultBrowser(wxString(_T("http://www.google.com/search?q=")) << lastWord);
  else if (id == idMsdn)
	wxLaunchDefaultBrowser(wxString(_T("http://search.microsoft.com/search/results.aspx?qu=")) << lastWord << _T("&View=msdn"));
  else
    event.Skip();
}
