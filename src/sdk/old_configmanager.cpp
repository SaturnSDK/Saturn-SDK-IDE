...
#include "old_configmanager.h" // class's header file
...
Configurations OldConfigManager::s_Configurations;

void OldConfigManager::Init(wxConfigBase* config)
{
...
wxConfigBase* OldConfigManager::Get()
{
    if (!ConfigBaseProxy::Get())
	{
        OldConfigManager::Init(wxConfigBase::Get());
		//Manager::Get()->GetMessageManager()->Log(_("OldConfigManager initialized"));
	}
    return ConfigBaseProxy::Get();
}

OldConfigManager::OldConfigManager()
{
}
...
void OldConfigManager::AddConfiguration(const wxString& desc, const wxString& key)
{
...
bool OldConfigManager::ExportToFile(const wxString& filename, int index)
{
...
bool OldConfigManager::ExportToFile(const wxString& filename, const ConfigurationPath& configuration)
{
...
bool OldConfigManager::ImportFromFile(const wxString& filename, int index)
{
    if (index < 0 || index >= (int)s_Configurations.Count())
        return false;
...
bool OldConfigManager::ImportFromFile(const wxString& filename, const ConfigurationPath& configuration)
{
    return true;
}
